...
namespace integraMobile.Infrastructure.RedsysAPI
{
    ...
    public class RedsysAPI
    {
     ...
     private byte[] Base64Decode(string encodedData)
     {
        ...
        /// <returns></returns>
        private Dictionary<string, object> FromJson(string json)
        {

            try
            {
                string strJson = json.Replace("\"{", "{").Replace("}\"", "}");
                Dictionary<string, object> res = JsonConvert.DeserializeObject<Dictionary<string, object>>(strJson);
                return res;
            }
            catch (JsonSerializationException ex)
            {
            ...
            {
                throw new JsonSerializationException(ex2.Message);
            }
        }
        ...
        public string createMerchantSignatureNotif(string key, string data)
        {
            ...
            // Convert JSON string to Dictionary
            foreach (var o in FromJson(deco))
            {
                m_keyvalues.Add(o.Key, o.Value.ToString());
            }
        ...
        public string GetParameterAll(bool bHtml)
        {
            string sRet = "";
            try
            {
                if (m_keyvalues != null)
                {
                    sRet = JsonConvert.SerializeObject(m_keyvalues, Formatting.None);
                    sRet = PrettyJSON(sRet, bHtml);
                }
            }
            catch (JsonSerializationException ex)
            {
        ...
        private string PrettyJSON(string json, bool bHtml = false)
        {
            string sLineTab = (bHtml ? "<br>&nbsp;&nbsp;&nbsp;&nbsp;" : "\r\n\t");
            string sLine = (bHtml ? "<br>" : "\r\n");

            try
            {
                dynamic parsedJson = JsonConvert.DeserializeObject(json);
                string strRes = JsonConvert.SerializeObject(parsedJson, Newtonsoft.Json.Formatting.Indented);
                return sLineTab + strRes.Replace(sLine, sLineTab ) + sLine;
            }
            catch
            {
                return sLineTab + json + sLine;
            }
        }
    }
}
