...
namespace integraMobile.Infrastructure.QrCodeNet.Rendering
{    
    public class QrRenderer
    {
        ...
        public static bool Render(string sCode, int iModuleSize, string sImagePath, ImageFormat imageFormat)
        {
            bool bRet = false;
            try
            {
                QrEncoder qrEncoder = new QrEncoder(ErrorCorrectionLevel.H);
                QrCode qrCode = qrEncoder.Encode(sCode);

                QrRenderer renderer = new QrRenderer(iModuleSize, Brushes.Black, Brushes.White);
                renderer.CreateImageFile(qrCode.Matrix, sImagePath, imageFormat);
