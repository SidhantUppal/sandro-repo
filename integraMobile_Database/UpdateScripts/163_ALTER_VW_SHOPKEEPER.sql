...
/****** Object:  View [dbo].[VW_SHOPKEEPER_USERS]    Script Date: 6/16/2020 12:16:21 PM ******/
SET ANSI_NULLS ON
GO
...
AND (dbo.USERS.USR_FIRST_OPERATION_INS_ID NOT IN (SELECT INS_ID FROM INSTALLATIONS INNER JOIN COUNTRIES_REDIRECTIONS ON INS_COU_ID = COURE_COU_ID) or dbo.USERS.USR_FIRST_OPERATION_INS_ID is null)
AND (dbo.USERS.USR_LAST_OPERATION_INS_ID NOT IN (SELECT INS_ID FROM INSTALLATIONS INNER JOIN COUNTRIES_REDIRECTIONS ON INS_COU_ID = COURE_COU_ID) or USERS.USR_LAST_OPERATION_INS_ID is null)
