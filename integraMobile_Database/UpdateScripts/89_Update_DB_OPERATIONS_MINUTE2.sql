...
BEGIN
	...
	DECLARE @FromMinuteLocal AS Datetime;
	SELECT @FromMinuteLocal = MIN(dateadd(minute, datediff(minute, 0, CASE WHEN OPE_TYPE = 1 OR OPE_TYPE = 2 OR OPE_TYPE = 3 THEN OPE_INIDATE 
	...
	WHERE --OPE_TYPE IN (1,2,3,4, 5) AND 
		  ...
		  ((CASE WHEN OPE_TYPE = 1 OR OPE_TYPE = 2 OR OPE_TYPE = 3 THEN OPE_UTC_INIDATE 
		    ...
		    CASE WHEN OPE_TYPE = 1 OR OPE_TYPE = 2 OR OPE_TYPE = 3 THEN OPE_UTC_INIDATE 
			   WHEN OPE_TYPE IN (5, 12) AND TRANS_STATUS = 7 THEN STATUS_DATE
			   ELSE OPE_UTC_DATE END <= GETUTCDATE()) OR
			CASE WHEN OPE_TYPE = 1 OR OPE_TYPE = 2 OR OPE_TYPE = 3 THEN OPE_INIDATE 
		       WHEN OPE_TYPE IN (5, 12) AND TRANS_STATUS = 7 THEN DATEADD(hour, -DATEDIFF(hour, OPE_DATE, OPE_UTC_DATE), STATUS_DATE) 
			   ELSE OPE_DATE END >= @FromMinuteLocal)
