...
<script type="text/javascript">
    ...
    function UsersSecurityOperations_Url(urlParameter, OpType) {
        if (OpType == 1) 
            return '<%= ConfigurationManager.AppSettings["UserSecurityOperations_ChangeEmail_Telephone_Url"] ?? "" %>' + urlParameter;
        else
            return '<%= ConfigurationManager.AppSettings["UserSecurityOperations_RecoverPassword_Url"] ?? "" %>' + urlParameter;
    }
