/*
* TelerikReporting v9.2.15.1216 (http://www.telerik.com/products/reporting.aspx)
...
(function(trv, $) {
    "use strict";
    var sr = {
        controllerNotInitialized: "Conrolador sense inicialitzar.",
        noReportInstance: "No report instance.",
        missingTemplate: "!obsolete resource!",
        noReport: "No report.",
        noReportDocument: "No report document.",
        invalidParameter: "Introdueixi un valor vàlid.",
        invalidDateTimeValue: "Introdueixi una data vàlida.",
        parameterIsEmpty: "El valor del paràmetre no pot estar buit.",
        cannotValidateType: "Cannot validate parameter of type {type}.",
        loadingFormats: "Carregant...",
        loadingReport: "Carregant informe...",
        preparingDownload: "Preparant el document per ser descarregat. Esperi...",
        preparingPrint: "Preparant el document per la seva impressió. Esperi...",
        errorLoadingTemplates: "Error carregant les plantilles del visor d'informes. (Plantilla = {0}).",
        loadingReportPagesInProgress: "{0} pàgines carregades ...",
        loadedReportPagesComplete: "Fet. {0} pàgines carregades.",
        noPageToDisplay: "No hi ha pàgines a mostrar.",
        errorDeletingReportInstance: "Error eliminant la instància de l'informe: {0}.",
		...
		errorCreatingReportInstance: " ",
        errorCreatingReportDocument: "Error creating report document (Report = {0}; Format = {1}).",
        unableToGetReportParameters: "Impossible obtenir els paràmetres de l'informe.",
		missingParameters: "Introdueixi els valors del parámetres de l'informe."
    };
    ...
    function ReportViewerController(options) {
        ...
        function ReportLoader(reportHost, useCache, baseDocumentId, actionId) {
            ...
            function loadAsync() {
                ...
                $.when(initializeClientAsync()).then(ajaxRequestAdapter(registerInstanceAsync, utils.stringFormat(sr.errorCreatingReportInstance, [ report ]))).then(ajaxRequestAdapter(function() {
                    return registerDocumentAsync(format, deviceInfo, useCache, baseDocumentId, actionId);
                }, utils.stringFormat(sr.errorCreatingReportDocument, [ report, format ]))).then(function(documentData) {
            ...
            if (xhr) {
                try {
                    ...
                    if (msg) {
                        ...
                        if (msg.exceptionMessage || msg.ExceptionMessage) {
                            
							if (msg.exceptionType == "Telerik.Reporting.Services.Engine.InvalidParameterException") {
								result = sr.missingParameters;
							}
							else {
								if (result) {
									result += "<br/>";
        ...
        function exportAsync(format, deviceInfo) {
            ...
            }, utils.stringFormat(sr.errorCreatingReportDocument, [ report, format ]))).then(ajaxRequestAdapter(function(documentData) {
        ...
        function loadParametersAsync(report, paramValues) {
            ...
            $.when(initializeClientAsync()).then(ajaxRequestAdapter(function() {
                ...
                d.reject(formatXhrError(xhr, status, error, errorMessage));
            });
            return d.promise();
        }
        ...
        $.extend(controller, {
            ...
            refreshReport: function(ignoreCache, baseDocumentId, actionId) {
                return loadReportAsync(ignoreCache, baseDocumentId, actionId);
            },
        ...
        function attach() {
            controller.beginLoadReport(function() {
                ...
                if (args.documentMapAvailable) {
                    ...
                    displayDocumentMap(args.bookmarkNodes);
                } else {
...
(function(trv, $, window, document, undefined) {
    ...
    trv.PerspectiveManager = function(dom, controller) {
        ...
        function init() {
            ...
            initializeState(perspectives[currentPerspective]);
        }
        function setPerspective(perspective) {
            initializeState(perspectives[perspective]);
            currentPerspective = perspective;
            if (currentPerspective) {
                applyState(perspectives[currentPerspective]);
            }
        }
...
(function(trv, $, window, document, undefined) {
    "use strict";
    if (!$) {
        alert("jQuery is not loaded. Make sure that jQuert is included.");
    }
    ...
    var templateCache = function() {
        ...
        function beginLoad(url, serviceUrl) {
            ...
            $.when($.get(url)).done(function(html) {
                ...
                cache[url] = templates;
                dfd.resolve(templates, styleSheets);
                loadComplete(url);
            }).fail(function() {
                dfd.reject();
                loadComplete(url);
    ...
    function ReportViewer(dom, options) {
        ...
        options = jQuery.extend({}, getDefaultOptions(options.serviceUrl), options);
        ...
        $.when(templateCache.load(options.templateUrl, options.serviceUrl)).done(function(t, styleSheets) {
            templates = t;
            loadStyleSheets(styleSheets);
            window.setTimeout(start, 100);
        }).fail(function() {
            $placeholder.html(utils.stringFormat(sr.errorLoadingTemplates, [ options.templateUrl ]));
        });
        return viewer;
    }
...
/* DO NOT MODIFY OR DELETE THIS LINE! UPGRADE WIZARD CHECKSUM 444D6047B929EFF575D4DB542704C320 */